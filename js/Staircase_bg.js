// edited by Becky Gilbert, July 2019
// Added new parameters 'stepChangeFactor' and 'stepSizeLimits', and a new option 'factor' for the 'operation' parameter. This allows for a changing step size, instead of just a static step size value. If specified, the stepChangeFactor will be multiplied by the previous step size to get the new step size at each value change. This differs from using step size up/down values with 'multiply', because in this latter case, the step size is multiplied/divided by the current value to get the new value. The 'stepChangeFactor' will change the step size unless the step size upper or lower limit is reached, in which case it stays the same.
...
function Staircase(stairs) {
  ...
  for (var i in stairs) {
    ...
    // Check minimum requirements: first value, and either stepSizeUp/stepSizeDown (add/multiply), or stepChangeFactor and firstStepSize (factor)
    ...
    this.stairs[i].maxTrialsAtMinVal = this.stairs[i].maxTrialsAtMinVal || false; // maximum number of trials at minimum difficulty (from 'limits') before triggering reversal
    ...
    this.stairs[i].isReversal = stairs[i].isReversal || []; // is this trial a reversal?
    this.stairs[i].isCorrResponse = stairs[i].isCorrResponse || []; // is the response for this trial correct?
