...
jsPsych.plugins['poldrack-survey-multi-choice'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    trial.timing_response = trial.timing_response || 30000; // default is 30s per trial
    trial.allowed_time = trial.allowed_time || 8*60; //default allowed time to finish the test is 8 minutes
    trial.warning = trial.warning || true //provide reminder before the trial is about to end
    ...
    //add conditional determining width of list elements depending on horizontal
      ...
      if (trial.warning==true){
          var t1 = setTimeout(function() {
            display_element.append("<p><font color='red'>10 seconds left</font></p>");
          }, trial.timing_response-10000);
          setTimeoutHandlers.push(t1);

      }
      //if resposne time is specified stop the trial
    ...
    function next(timeout=false) {
      var Timediff = Math.abs(t0 - new Date());
      var second = Math.floor((Timediff/1000.0));
      //console.log(second)
      ...
      if(check_required_questions()||timeout==true){
        if (timeout==true){
          alert('Please be faster to respond');
        }
        add_current_page_to_view_history();

        //submit_page_data();
        //DON'T SUBMIT IT BUT UPDATE THE RESPONSE_DATA ARRAY
        update_page_data(timeout=timeout);

        current_page++;

        // if done, finish up...
        if (current_page >= trial.pages.length || second>= trial.allowed_time) { 
    ...
    function update_page_data(timeout=false){
      ...
      for (var i = 0; i<trial.pages[current_page].length; i++){
        ...
        if(trial.input_type[current_page][i] == 'radio'){
          ...
          response_data[current_page][i] = 'timeout'
        }else{response_data[current_page][i] = $('#'+option_selector).find('input:radio:checked').val()}
        }
